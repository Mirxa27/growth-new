...
export const AIAssessmentBuilder: React.FC<AIAssessmentBuilderProps> = ({ 
  ...
  const generateAIAssessment = async () => {
    ...
    try {
      // Validate input data
      const validatedData = validateData(CreateAssessmentSchema, {
      ...
      const response = await supabase.functions.invoke('ai-assessment-generator', {
        body: {
          type: type,
          topic: aiPrompt,
          difficulty: 'medium',
      ...
      if (response.data) {
        ...
        logger.info('AI assessment generated successfully', 'AIAssessmentBuilder', {
          type,
          questionCount: gen.questions?.length || 0
        });
      ...
      const appError = errorHandler.handleError(error, 'AIAssessmentBuilder');
      logger.error('AI assessment generation failed', 'AIAssessmentBuilder', appError);
  ...
  const saveAssessment = async () => {
    setLoading(true);
    try {
      // Validate the complete assessment data
      ...
      const validatedData = validateData(CreateAssessmentSchema, assessmentData);

      // Prepare questions in the correct format for the stored procedure
      const formattedQuestions = validatedData.questions.map((q, index) => ({
      ...
      const { data, error } = await supabase.rpc('create_assessment_with_questions', {
        ...
        _category: validatedData.category,
        _visibility: validatedData.visibility,
        _estimated_time: validatedData.estimatedTime,
        _ai_provider: 'openai',
        _ai_model: 'gpt-4o-mini',
        _ai_prompt: aiPrompt || `Manual assessment: ${validatedData.title}`,
        _questions: formattedQuestions,
        _scoring: validatedData.scoring
      });

      if (error) throw error;

      logger.info('Assessment created successfully', 'AIAssessmentBuilder', {
      ...
      const appError = errorHandler.handleError(error, 'AIAssessmentBuilder');
      logger.error('Assessment save failed', 'AIAssessmentBuilder', appError);
