...
CREATE TABLE IF NOT EXISTS public.admin_ai_providers (
  ...
  available_voices JSONB DEFAULT '[]',
...
-- Voice sessions table (for tracking voice conversations)
...
-- Insert sample assessment questions and options
DO $$
DECLARE
...
BEGIN
  ...
  IF assessment_id IS NOT NULL THEN
    ...
    FOR question_id IN SELECT id FROM public.assessment_questions WHERE assessment_id = assessment_id LOOP
      IF (SELECT position FROM public.assessment_questions WHERE id = question_id) = 1 THEN
...
INSERT INTO public.voice_agent_configs (
  ...
  'You are NewMe, a supportive AI companion for personal growth and self-discovery.',
  0.7,
  1000,
  true
WHERE NOT EXISTS (SELECT 1 FROM public.voice_agent_configs WHERE user_id = auth.uid());
...
DO $$
BEGIN
  RAISE NOTICE 'Growth Echo Nexus database setup completed successfully!';
  RAISE NOTICE 'Tables created: profiles, assessments, assessment_questions, assessment_options, assessment_results';
  RAISE NOTICE 'AI Provider tables: admin_ai_providers, voice_agent_configs, voice_sessions';
  RAISE NOTICE 'Community tables: posts, post_likes, post_comments';
  RAISE NOTICE 'Library tables: library_items, user_library_progress';
  RAISE NOTICE 'Sample data inserted with public assessment and library items';
  RAISE NOTICE 'RLS policies configured for security';
  RAISE NOTICE 'Essential functions created for automation';
END $$;
